<?php
    ...
    $query = "SELECT min(rok_produkcji), round(avg(rok_produkcji),0), max(rok_produkcji), count(id) FROM `spis_taboru` WHERE `typ` = 'AUTOBUS' and `przewoznik` = '$przewoznik'";
  
    if ($result = @$connect->query($query)) {
        $statistic = $result->num_rows;
        if ($statistic > 0) {
            while ($row = $result->fetch_array()) {
                echo "<H4>Autobusy:</H4><br/>";
